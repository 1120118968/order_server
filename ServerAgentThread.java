...
public class ServerAgentThread extends Thread{//创建一个叫ServerAgentThread的继承线程的类
		...
		public void run(){//重写run方法
			try{
				...
				System.out.println("readinfo==="+readinfo);//打印 从流中 读取的数据
		//================Pc端请求标识头 begin====================
				...
				readinfo=IOUtil.readStr(in);//将从流中读取的数据放入字符串中
		...
		//--------------------------------------begin（PC端前台）			
		   ...
		   //根据菜品id得到这个菜的所有子图
			...
			//服务员手持端得到菜品信息
				...
				String[] getstr=new String [5];//readinfo.split(GET_W_VEGEINFO);
		
				List<String[]> vegelist=DBUtil.getWVege(getstr[0]);
				String strinfo=TypeExchangeUtil.listToString(vegelist);
				IOUtil.writeStr(out, MyConverter.escape(strinfo));
			}
		   //根据菜品id得到菜品的相关信息
			...
			//根据员工id得到员工姓名
				...
				String[] getinfo=new String[5];//readinfo.split(GET_WORKERNAMEBYID);
				getinfo[1] = GET_WORKERNAMEBYID;
